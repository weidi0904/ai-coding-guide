...
这张图把两个阶段并排摆开：左边阶段一，启动时每个 Skill 只占「名称 + 一句描述 + 文件路径」，初始清单总共约占上下文的 2%，正文一概不读；右边阶段二，只有被任务命中 `description` 的那一个 Skill 才把完整 `SKILL.md`（含正文指令）展开读入，其余照旧躺在清单里只占一行——省上下文的秘密，全在这「清单常驻、全文按需加载」上。
