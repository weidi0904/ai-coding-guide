...
- **沙箱模式（sandbox mode）** 管「能动多大」：配置键 `sandbox_mode`，命令行 `--sandbox`（简写 `-s`）。
- **审批策略（approval policy）** 管「问不问你」：配置键 `approval_policy`，命令行 `--ask-for-approval`（简写 `-a`）。
...
沙箱旋钮一共三档，官方原话定义很干脆，我用「能不能改文件 / 能不能联网 / 配哪种活」三栏给你摆平——**这是本节最该记住的一张表**：
...
**第二，工作区不只是「当前目录」。** 官方说工作区还**自动包含临时目录**（比如 `/tmp`）。具体哪些目录在工作区内，进会话敲 `/status` 一看便知，别瞎猜。
...
至于「谁来审批」，默认是弹给你本人（`approvals_reviewer = "user"`）。官方还提供了一个 `auto_review`（自动审查）选项——让一个审查 agent 替你先过一遍那些需要审批的请求。这个属于偏进阶、还在演进的能力，**第 16 篇「安全与风险边界」会展开讲它的判断逻辑和风险**，这里你只要知道有这么个开关，日常 `user` 够用。
...
第 02 篇你可能注意到我特意提醒过：别把别的教程里那套 `ask` / `approve` / `deny` 当成 Codex 的官方叫法——**Codex 真正用来「精确控单条命令」的机制叫 rules（规则）**，写法也不一样。
...
还有一条和「联网」相关的默认值，顺手记一下——**网页搜索（web search）默认走缓存（`cached`），不是实时抓取**。官方维护了一份预索引的结果，缓存模式返回的是这份索引，而不是去抓活页面。好处是**降低被「实时页面里藏的提示注入」攻击的风险**。注意一个反直觉点：**如果你开了完全访问（`--yolo` 那套），网页搜索反而会默认切成实时（live）**。想强制实时用 `--search`、想彻底关掉设 `web_search = "disabled"`，但这块的安全考量留到第 16 篇细说。
...
- 命令行一把梭：`--dangerously-bypass-approvals-and-sandbox`，别名就叫 `--yolo`

`--yolo` 这名字（you only live once，「人生只有一次」）就差把「别瞎用」写脸上了。它**既拆掉沙箱、又关掉审批**，等于让 Codex 在你整台机器上为所欲为、还一句都不问。该不该开，照这张表对号：
