...
如果你从 Claude Code 篇一路读过来，对这套应该不陌生——`AGENTS.md` 之于 Codex，就是 `CLAUDE.md` 之于 Claude Code，**同一个概念换了个文件名**。但 Codex 这边的发现机制、覆写规则、字节上限，跟 Claude Code 有几处实打实的不同，照搬经验会栽跟头。这篇就把这些差异连同「该写什么、不该写什么」一次讲清。
