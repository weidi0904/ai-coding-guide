...
这里有个关键词——**标准**。MCP 不是 OpenAI 关起门来自己玩的私有协议，而是一套公开规范。**好处是「一次对接，到处能用」**：你给某个工具写的 MCP server，Codex 能用，别的支持 MCP 的客户端（Claude Code、Cursor……）也能用。如果你 Claude Code 篇一路读过来，这套你应该眼熟——**同一个 MCP 协议，Codex 这边也认**，只是配法不同（这正是开头我栽跟头的地方，下面第 03 节专门讲）。
...
这里得跟从 Claude Code 过来的兄弟点一句**最容易想当然的差异**：Claude Code 那边还有个「已弃用的 SSE 传输」要你提防，**Codex 官方文档列的就 STDIO 和 Streamable HTTP 两种**，干净利落，你不用纠结 SSE。记住这两种就够了。
