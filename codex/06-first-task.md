...
> 💡 **一句话总结**：第一次用 Codex，先建个三行代码的玩具当「练车场」——**它改了啥你一眼看得清，改砸了删掉重建，全程心不慌**。
...
为什么先聊这个？因为 Codex 不是许愿池——你扔进去什么质量的需求，它就回你什么质量的活。[02 核心概念](02-core-concepts.md) 讲过它是个**代理（Agent）**，干活靠一个循环：官方大意是它「调用模型，然后执行模型指示的动作，比如读文件、改文件、调工具，直到任务完成或被你取消」。翻成大白话还是那三个动作——**想 → 做 → 看**。
...
| 跑一条命令（如测试、装包） | 出圈或不可信时停下来问 | 「同意 / 拒绝」让你点 |
