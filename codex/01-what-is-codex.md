...
> 📚 **系列导航**：这是「Codex 小白教程」的**开篇第一篇**。如果你是从隔壁「Claude Code 篇」一路读过来的，那正好——两个工具一个出身 OpenAI、一个出身 Anthropic，理念像、用法各有门道，对照着学最省力。下一篇〈02 核心概念速览〉，咱们把术语一次性捋顺。

兄弟们，今天换个主角，聊 OpenAI 的 Codex。

我猜你的状态是这样：刚把 Claude Code 摸熟，或者压根还没碰过 AI 编程工具，突然又冒出来个 Codex，名字听着耳熟（OpenAI 几年前就有个叫 Codex 的老模型），打开它官网一看更懵——**怎么又是桌面 App、又是命令行、又是浏览器插件、还有个云端网页版？到底哪个才是「Codex 本体」？我该装哪个？**

说句实话，这正是 Codex 最容易把新手绕晕的地方：它不是「一个软件」，而是**同一个 AI 编程代理（Agent），换了四副面孔**。搞不清这点，你会在「该下载哪个」上卡半天。
...
这种「外包」模式有两个实打实的好处，菜鸟教程总结得挺到位：**一是并行**——你可以一口气甩出去三五个任务同时跑，互不阻塞；**二是隔离**——它在自己的沙箱里折腾，不碰你本机系统，搞砸了也波及不到你。我自己最常用的搭配是：手头小修小补用桌面 App 或 CLI 当场盯着，「跑全套测试再修好失败用例」这种又慢又长的活，丢去云端挂着，过会儿回来收。
...
最容易踩的是第二条。我去年底干过一件蠢事：让 Codex 给一个项目批量补错误处理，它刷刷改了十几个文件，我图省事扫都没扫就提交了。结果有两处它「自作主张」顺手改了原有逻辑，过了好几天才发现一个接口行为变了。**从那以后我立了条铁规矩：它改的每一行，至少过一遍 diff 再提交。** 顺嘴提一句，Codex 官方在快速上手里也反复劝你——动手前后用 Git 打个检查点（checkpoint，可以理解成给项目「存档」），万一改砸了一键回滚。这个习惯，新手越早养成越好。

用好 Codex 的心态，其实就一句话：

> 让 Codex 提供高质量候选方案，而不是绝对正确答案。

说白了——**人定方向、把关、做判断；AI 负责执行、分析、干重复劳动**。这是它的设计哲学：协作，不是替代。这套心态跟 Claude Code 完全一致，从隔壁篇过来的兄弟应该很眼熟。
...
说白了：**ChatGPT 你问它答，Codex 你下指令它动手干。** 而且有意思的是——Codex 本身就含在 ChatGPT 的订阅里（按官方文档，Plus / Pro / Business / Edu / Enterprise 这几档订阅都包含 Codex 用量），你也可以用 OpenAI 的 API key 按量付费来登。具体哪档订阅给多少额度、各功能在不同登录方式下有没有差异，**变化较快，一律以官方计费页为准**（这块第 04 篇专门讲）。

### Codex 和 Claude Code 差在哪

这是从 Claude Code 篇过来的兄弟最关心的。先泼盆冷水：**别问「哪个更强」，这俩定位太像了，强弱要看你具体怎么用。** 真正的差别在「工作流重心」：
